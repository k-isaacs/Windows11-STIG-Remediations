<#
.SYNOPSIS
    Configures the Windows 11 Security event log maximum size to meet STIG WN11-AU-000505.
...
.NOTES
    ...
    STIG Page       : https://stigaview.com/products/win11/v2r5/WN11-AU-000505/
...
.TESTED ON
    Date(s) Tested  : 2026-04-06, 2026-04-08
...
$policyPath = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\EventLog\Security'
$policyName = 'MaxSize'
$requiredKB = 1024000
...
if ($configuredValue -eq $requiredKB) {
    Write-Host "COMPLIANT"
    Write-Host "Registry ${policyName} (KB): $configuredValue"
    exit 0
}
# Remediation
if ($makeCompliant) {
    New-Item -Path $policyPath -Force | Out-Null
    New-ItemProperty -Path $policyPath -Name $policyName -PropertyType DWord -Value $requiredKB -Force | Out-Null
    Write-Host "Security log maximum size policy has been configured."
} else {
